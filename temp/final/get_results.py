...
commits_dataset = pd.read_csv('commits_dataset_of_1064_unique.csv')

pulls_dataset = pd.read_csv('pulls_dataset_of_1064.csv')
...
for i_index, i in pulls_dataset.iterrows():
    ...
    df_r = df_r.drop_duplicates(ignore_index=True)
    df_r.to_csv('first_ten_final_one/first_ten_'+user_name+'_details.csv',sep=',', encoding='utf-8')
    df_list.append(df_r)
    #print(i,': completed')
            #j0=j0+1
            #cc=cc+1
...
total_df.to_csv('first_ten_final_total_1000.csv',sep=',', encoding='utf-8')
